...
def binary_classifier_metrics(classifier, Xtrain, Ytrain, Xtest, Ytest, classes, outcome):
    ...
    # report
    print(metrics.classification_report(Ytest, predicted_class,  labels=[0,1], target_names=classes))
...
def train_test_regressor(Xdata, Ydata, regressor, clf_name, pars, score, outcome, 
                         classes=['NO', 'YES'], oversampling=False, k=5, cv=3):
